...
class App(QWidget):
	...
	def initUI(self):
		...
		opp = Opponent(seconds_per_move=seconds_per_move,search_n=search_n)
		self.env = GoGame(size=9, oppo_thread=opp,width=self.width*boardFrac*0.95, height=self.height*boardFrac*0.95, condition=cond, reset_clock=self.digit_clock.resetTime)
